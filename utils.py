...
import operator, math, random, copy, sys, os.path, bisect, functools, search
...
class CompGen():
    def __init__(self, node, cont):
        self.node = node
        self.cont = cont

    def path(self):
        """Create a list of nodes from the root to this node."""
        x, result = self.node, [self.node]
        while x.parent:
            result.append(x.parent)
            x = x.parent
        result.append("Con " + str(self.cont) + " nodos expandidos.")
        return result
